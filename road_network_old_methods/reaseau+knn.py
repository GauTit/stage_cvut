...
if len(green_stations) > 1:
    ...
    G_knn = nx.Graph()
    n = len(coords)
    G_knn.add_nodes_from(range(n))
    for i in range(n):
        for j_idx in indices[i][1:]:   # on ignore le premier (= soi-même)
            dist_ij = np.linalg.norm(coords[i] - coords[j_idx])
            G_knn.add_edge(i, j_idx, weight=dist_ij)
            
        
    knn_group = folium.FeatureGroup(name=f"{k}-NN Stations Vertes")
    for u, v, d in G_knn.edges(data='weight'):
