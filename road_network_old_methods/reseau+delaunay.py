...
green_stations = stations_proj[stations_proj['dist_to_highway_m'] <= 1000]
...
if len(green_stations) > 1:
    ...
    # Récupération des coordonnées en Lambert-93
    coords = np.array([(pt.x, pt.y) for pt in green_stations.geometry])

    from scipy.spatial import Delaunay

    # Coordonnées en Lambert-93
    pts = np.array([(pt.x, pt.y) for pt in green_stations.geometry])
    tri = Delaunay(pts)

    # Construction du graphe Delaunay
    G_del = nx.Graph()
    n = len(pts)
    G_del.add_nodes_from(range(n))
    for simplex in tri.simplices:
        for u, v in [(simplex[i], simplex[j]) for i in range(2) for j in range(i+1,3)]:
            dist_uv = np.linalg.norm(pts[u] - pts[v])
            G_del.add_edge(u, v, weight=dist_uv)
        
    knn_group = folium.FeatureGroup(name=f"{k}-NN Stations Vertes")
    for u, v, d in G_del.edges(data='weight'):
        u_geom = stations_gdf.loc[green_stations.index[u]].geometry
        v_geom = stations_gdf.loc[green_stations.index[v]].geometry
        folium.PolyLine(
            locations=[(u_geom.y, u_geom.x), (v_geom.y, v_geom.x)],
            color='orange', weight=5, opacity=0.8,
            tooltip=f"{d:.0f} m"
        ).add_to(knn_group)
    knn_group.add_to(m)
